...
import uk.co.datacable.app.xls.SupplierDataTransfer;

@WebServlet("/admin/newSupplierList")
@MultipartConfig(fileSizeThreshold = 1024 * 1024 * 10, // 10 MB
maxFileSize = 1024 * 1024 * 100, // 50 MB
maxRequestSize = 1024 * 1024 * 100)
// 100 MB
public class NewSupplierList extends AbstractServletHandler {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		gotoToJSP("admin/newSupplierList.jsp", req, resp);
	}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		for (Part part : req.getParts()) {
			...
			if (name.endsWith(".csv") || name.endsWith(".CSV")) {
				...
				SupplierDataTransfer.transfer(uploadFilePath + File.separator + fileName);
